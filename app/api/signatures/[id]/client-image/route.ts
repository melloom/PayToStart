...
export async function GET(
  ...
  try {
    ...
    const { clientSignature } = await getAllSignatures(contractId);
    
    if (!clientSignature) {
      return NextResponse.json(
        { error: "No client signature found" },
        { status: 404 }
      );
    }

    // Check for signature_url in both snake_case and camelCase
    const signatureUrl = (clientSignature as any).signature_url || (clientSignature as any).signatureUrl;
    
    if (!signatureUrl || (typeof signatureUrl === 'string' && signatureUrl.trim() === "")) {
      return NextResponse.json(
        { error: "No client signature image available" },
        { status: 404 }
      );
    }
    ...
    console.error("Error serving client signature image:", error);
    return NextResponse.json(
...
async function getFreshSignedUrl(signatureUrl: string, contract: any): Promise<NextResponse> {
  try {
    console.log("Getting fresh signed URL for:", signatureUrl);
