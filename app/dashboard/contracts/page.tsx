...
export default async function ContractsPage({
  ...
  // Fetch contracts and drafts in parallel
  const [contracts, drafts] = await Promise.all([
    db.contracts.findByContractorId(contractor.id),
    db.contractDrafts.findByContractorId(contractor.id),
  ]);

  // Get clients for all contracts - fetch in a single batch query
  const uniqueClientIds = [...new Set(contracts.map(c => c.clientId).filter(Boolean))];
  const clientsMap = new Map();
  
  // Fetch all clients in a single query instead of multiple individual queries
  if (uniqueClientIds.length > 0) {
    const clients = await db.clients.findByIds(uniqueClientIds);
